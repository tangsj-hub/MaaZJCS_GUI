export default {
  ...
  settings: {
    ...
    adbExecutablePath: 'ADB 実行ファイルのパス',
    adbExecutablePathHint:
      'ここで設定した場合のみ、自動検出はこの ADB パスで `adb devices` を追加実行して実機を検出します。空欄の場合は従来どおりエミュレータ検出ロジックのみを使用します。',
    adbExecutablePathPlaceholder: '例: <Android SDK>/platform-tools/adb',
  ...
  taskItem: {
    ...
    renameTask: 'タスク名を変更',
    loopCountTitle: 'ループ設定',
    loopCountMessage: 'このタスクのループ回数と間隔を設定します',
    loopCountLabel: 'ループ回数',
    loopCountHint: '1 = ループなし',
    loopDelayLabel: '間隔時間（ミリ秒）',
    loopDelayHint: '各ループ後の待機時間、0 = 待機なし',
