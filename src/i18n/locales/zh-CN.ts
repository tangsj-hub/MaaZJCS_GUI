export default {
  ...
  settings: {
    ...
    adbExecutablePath: 'ADB 可执行文件路径',
    adbExecutablePathHint:
      '仅在这里配置后，自动检测才会额外使用该 ADB 路径执行 `adb devices` 发现真机；留空时保持原有模拟器检测逻辑。',
  ...
  taskItem: {
    ...
    renameTask: '重命名任务',
    loopCountTitle: '循环设置',
    loopCountMessage: '设置该任务的循环执行次数和间隔时间',
    loopCountLabel: '循环次数',
    loopCountHint: '1 = 不循环',
    loopDelayLabel: '间隔时间（毫秒）',
    loopDelayHint: '每次循环结束后的等待时长，0 = 不等待',
