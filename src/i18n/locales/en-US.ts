export default {
  ...
  settings: {
    ...
    adbExecutablePath: 'ADB executable path',
    adbExecutablePathHint:
      'Only when configured here will auto-detect additionally run `adb devices` via this ADB path to discover physical devices. Leave empty to keep the original emulator-only detection logic.',
    adbExecutablePathPlaceholder: 'e.g. <Android SDK>/platform-tools/adb',
  ...
  taskItem: {
    ...
    loopCountTitle: 'Loop Settings',
    loopCountMessage: 'Set the loop count and delay for this task',
    loopCountLabel: 'Loop Count',
    loopCountHint: '1 = no loop',
    loopDelayLabel: 'Delay (ms)',
    loopDelayHint: 'Wait time after each loop iteration, 0 = no wait',
    loopCountBadge: '×{{count}}',
    loopDelayBadge: '{{delay}}ms',
