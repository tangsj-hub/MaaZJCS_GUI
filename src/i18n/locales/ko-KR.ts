export default {
  ...
  settings: {
    ...
    adbExecutablePath: 'ADB 실행 파일 경로',
    adbExecutablePathHint:
      '여기에 설정한 경우에만 자동 감지가 이 ADB 경로로 `adb devices` 를 추가 실행하여 실기기를 찾습니다. 비워 두면 기존 에뮬레이터 감지 로직만 사용합니다.',
    adbExecutablePathPlaceholder: '예: <Android SDK>/platform-tools/adb',
  ...
  taskItem: {
    ...
    renameTask: '작업 이름 변경',
    loopCountTitle: '반복 설정',
    loopCountMessage: '이 작업의 반복 횟수와 간격을 설정합니다',
    loopCountLabel: '반복 횟수',
    loopCountHint: '1 = 반복 없음',
    loopDelayLabel: '간격 시간 (밀리초)',
    loopDelayHint: '각 반복 후 대기 시간, 0 = 대기 없음',
