export default {
  ...
  settings: {
    ...
    adbExecutablePath: 'ADB 可執行檔路徑',
    adbExecutablePathHint:
      '僅在此處設定後，自動偵測才會額外使用該 ADB 路徑執行 `adb devices` 來發現真機；留空時維持原本的模擬器偵測邏輯。',
    adbExecutablePathPlaceholder: '例如：<Android SDK>/platform-tools/adb',
  ...
  taskItem: {
    ...
    renameTask: '重新命名任務',
    loopCountTitle: '循環設定',
    loopCountMessage: '設定該任務的循環執行次數和間隔時間',
    loopCountLabel: '循環次數',
    loopCountHint: '1 = 不循環',
    loopDelayLabel: '間隔時間（毫秒）',
    loopDelayHint: '每次循環結束後的等待時長，0 = 不等待',
  ...
  debug: {
    ...
    verbosePipelineLog: '流水線詳細日誌',
    verbosePipelineLogHint:
      '在任務日誌中輸出節點辨識、動作與 next 列表（含 JSON 摘要），便於查看卡在哪個節點',
    saveDraw: '儲存除錯圖像',
